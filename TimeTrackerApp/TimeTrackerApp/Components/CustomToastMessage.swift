...
class CustomToastMessage {
    ...
    static func show(
        ...
        for item in controller.view.subviews where item.tag == 12212121212121 {
        ...
        let toastViewTwo = NSLayoutConstraint(
            item: toastView,
            attribute: .trailing,
            relatedBy: .equal,
            toItem: controller.view,
        ...
        let toastViewThree = NSLayoutConstraint(
            item: toastView,
            attribute: (showIn == .top) ? .top : .bottom,
            relatedBy: .equal, toItem: controller.view,
            attribute: (showIn == .top) ? .top : .bottom,
            multiplier: 1,
            constant: (showIn == .top) ? controller.view.safeAreaInsets.top + 16 : -(controller.view.safeAreaInsets.bottom + 80)
        )
        controller.view.addConstraints([toastViewOne, toastViewTwo, toastViewThree])
